...
# path to folder mp3 files
path_in_mp3 = 'D:\\Management-PRODIJ\\ProMassaPy\\vosk\\audios\\'
# path to folder wav files (converted to wav but not downsample, samplerate = sample rate mp3 file)
path_out_48 = 'D:\\Management-PRODIJ\\ProMassaPy\\vosk\\audios\\wav\\'
# path to folder wav files (converted to wav and downsample, samplerate = outrate)
path_out_16 = 'D:\\Management-PRODIJ\\ProMassaPy\\vosk\\audios\\wav\\'
for file in os.listdir(path_in_mp3):
    if file.endswith(".mp3"):
        print(file)
        sound = pydub.AudioSegment.from_mp3(
            path_in_mp3 + file)  # load mp3 file
        sound.export(path_out_48+file.replace('.mp3', '.wav'),
                     format="wav")  # export to wav file
        # downsample wav file to samplerate you want...
        downsampleWav(path_out_48+file.replace('.mp3', '.wav'),
                      path_out_16 + file.replace('.mp3', '.wav'), 48000, 16000, 1, 1)
